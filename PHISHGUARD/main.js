...
function getPackagedBackend() {
    const exeName = process.platform === 'win32'
        ? 'phishguard-backend.exe'
        : 'phishguard-backend';
    const exePath = path.join(process.resourcesPath, 'backend', exeName);
    if (!fs.existsSync(exePath)) {
        throw new Error(`Bundled backend not found at ${exePath}`);
    }
    return {
        command: exePath,
        args: [],
...
async function startFlask() {
    ...
    const launchers = app.isPackaged
        ? [getPackagedBackend()]
        : findPython().map((pyCmd) => ({
            command: pyCmd,
    ...
    for (const launcher of launchers) {
        try {
            ...
            console.log(`Failed to start with ${launcher.label}: ${e.message}`);
        }
    }

    throw new Error(app.isPackaged
        ? 'Could not start the bundled PhishGuard backend.'
        : 'Could not find a supported Python runtime. Please install Python 3.10+');
}
...
app.whenReady().then(async () => {
    try {
        ...
        dialog.showErrorBox('PhishGuard', `Failed to start: ${err.message}\n\nIn development, make sure Python 3.10+ is installed with the packages from requirements.txt.`);
        app.quit();
    }
});
