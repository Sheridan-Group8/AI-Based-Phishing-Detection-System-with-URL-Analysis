...
# Known-good SHA-256 of the shipped phishing_model.pkl. Update this when
# intentionally retraining. Can also be overridden at runtime via the
# PHISHGUARD_MODEL_SHA256 env var (useful for dev) or the expected_sha256
# argument to load_model.
_DEFAULT_MODEL_SHA256 = "cde9377a507d3a301b6c46675bc9b00ad1e91adc5875d12feb33403bf893618f"
...
class URLAnalyzer:
    ...
    # Cap how much text we hand to the regex per call — bodies above this are
...
class PhishingDetector:
    ...
    def load_model(self, filepath='phishing_model.pkl', expected_sha256=None):
        """Load both text and URL models from disk.

        Before unpickling, verify that the file's SHA-256 matches the
        expected digest. This catches tampered artifacts and prevents
        arbitrary-code execution via a replaced pickle. The digest can be
        provided by the caller; otherwise we fall back to
        PHISHGUARD_MODEL_SHA256 env var, then to _DEFAULT_MODEL_SHA256.

        Set expected_sha256='' to skip verification (dev/test only).
        """
        expected = expected_sha256
        if expected is None:
            expected = os.environ.get("PHISHGUARD_MODEL_SHA256", _DEFAULT_MODEL_SHA256)
        if expected:
            h = hashlib.sha256()
            with open(filepath, 'rb') as f:
            ...
            if not hmac.compare_digest(actual, expected):
                raise ValueError(
                    ...
                    f"Refusing to unpickle potentially tampered artifact."
                )
        with open(filepath, 'rb') as f:
            data = pickle.load(f)
